...
Zigbee mode was not available in early versions of firmware (despite the manual!). Even in October 2015 "old firmware is still
shipped in some stock being supplied. 

Version 1.71 or later is recommended which allows a wifi web interface to facilitate configuration with zigbee mode enabled.
The web interface also allows switching between matter and zigbee modes.
